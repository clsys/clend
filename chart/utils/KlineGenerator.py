...
from chart.constant import Freq
...
class KlineGenerator:
    ...
    def init_from_db(self, freq, barlist: List[RawBar] = []):
        if freq == "1m":
            self.m1 = barlist
        if freq == "5m":
            self.m5 = barlist
    ...
    def __update_d(self, k=None):
        ...
        k = RawBar(symbol=k.symbol, id=1, freq=Freq.D, dt=next_end_dt, open=k.open, close=k.close, high=k.high,
                   low=k.low, vol=k.vol)
        if not self.D:
            self.D.append(k)
        last = self.D[-1]
    ...
    def get_kline(self, freq: str, count: int = 1000) -> List[RawBar]:
        ...
        return freqs_map[freq][0:count]

    def get_all_kline(self, freq: str) -> List[RawBar]:
        """获取单个级别的K线
        ...
        #              "60分钟": self.m60, "日线": self.D, "周线": self.W, "月线": self.M}
        freqs_map = {"1分钟": self.m1, "5分钟": self.m5, "15分钟": self.m15, "30分钟": self.m30,
                     "60分钟": self.m60, "日线": self.D}
        # return freqs_map[freq][-count:]
        return freqs_map[freq]
